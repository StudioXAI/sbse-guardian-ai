    ...
    <div className="relative overflow-hidden rounded-[36px] border border-white/10 bg-black p-8 shadow-2xl">
      ...
      <div className="relative z-10">
        ...
        <p className="text-white/70 text-lg mb-10">
          Advanced Smart Contract, Token, Liquidity & Investor Protection Analysis
        </p>
        ...
        <div className="grid gap-4 md:grid-cols-4 mb-8">
          {metricCards.map((card) => (
            ...
            >
              <p className="text-xs text-white/60 mb-3">{card.title}</p>
              <div className="flex items-end gap-1">
                <h3 className="text-4xl font-bold text-white">{card.value}</h3>
                <span className="text-white/50 mb-1">{card.suffix}</span>
              </div>
              <p className="text-sm text-green-400 mt-2">{card.status}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="rounded-3xl border border-white/10 bg-black/70 p-8">
            ...
            <div className="flex items-center justify-center py-8">
              <div className="relative h-[340px] w-[340px] flex items-center justify-center">
                ...
                <div className="absolute inset-10 rounded-full bg-black border border-white/10 flex items-center justify-center text-center">
                  <div>
                    <p className="text-white/50 text-sm">TOTAL FINDINGS</p>
          ...
          {/* Security layers */}
          <div className="rounded-3xl border border-white/10 bg-black/70 p-8">
            <h3 className="text-2xl font-bold text-white mb-6">Security Layer Scores</h3>

            <div className="space-y-5">
              {securityLayers.map(([title, score, label, width]) => (
                <div key={title}>
