/* eslint-disable */
/* ─────────────────────────────────────────────────────────────
   ERC-20 Source Bundle for Etherscan Verification

   Embeds the exact OpenZeppelin v5.0.0 source files used to
   compile the StandardERC20 bytecode. These get submitted to
   Etherscan as JSON Standard Input format for source verification.

   IMPORTANT: do NOT modify these strings. They must be byte-for-byte
   identical to what Hardhat used at compile time, otherwise the
   verifier recompiles them and gets different bytecode → fails.

   The keys in SOURCE_FILES match the import paths Solidity uses
   in StandardERC20.sol (e.g. "@openzeppelin/contracts/...").
   ───────────────────────────────────────────────────────────── */

/** contracts/StandardERC20.sol (626 bytes) */
const STANDARD_ERC20_SOL = `// SPDX-License-Identifier: MIT
...
/** @openzeppelin/contracts/token/ERC20/ERC20.sol (10800 bytes) */
const OZ_ERC20_SOL = `// SPDX-License-Identifier: MIT
...
/** @openzeppelin/contracts/token/ERC20/IERC20.sol (2781 bytes) */
const OZ_IERC20_SOL = `// SPDX-License-Identifier: MIT
...
/** @openzeppelin/contracts/token/ERC20/extensions/IERC20Metadata.sol (684 bytes) */
const OZ_IERC20METADATA_SOL = `// SPDX-License-Identifier: MIT
...
/** @openzeppelin/contracts/utils/Context.sol (963 bytes) */
const OZ_CONTEXT_SOL = `// SPDX-License-Identifier: MIT
...
/** @openzeppelin/contracts/access/Ownable.sol (3102 bytes) */
const OZ_OWNABLE_SOL = `// SPDX-License-Identifier: MIT
...
/** @openzeppelin/contracts/interfaces/draft-IERC6093.sol (6534 bytes) */
const OZ_IERC6093_SOL = `// SPDX-License-Identifier: MIT
...
/**
 * Map of import path → source code, suitable for Etherscan's
 * JSON Standard Input format verification request.
 */
export const SOURCE_FILES: Record<string, string> = {
  "contracts/StandardERC20.sol": STANDARD_ERC20_SOL,
...
/**
 * Standard JSON Input for solc + Etherscan verifier.
 * Compiler settings here MUST match what produced the bytecode.
 */
export function buildStandardJsonInput(): string {
...
/** Compiler version string Etherscan expects (matches Hardhat compile). */
